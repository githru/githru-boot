...
const TimeSeries = ({ width, height }) => {
  ...
  const getURLData = async () => {
      const res = await d3.csv(csvURL).then((res) => res.map((d) => ({ date: d3.timeParse("%Y-%m-%d")(d.date as string) as Date, value: parseFloat(d.value as string)})));
      setData(res);
  } 
  ...
  const drawChart = () => {
    ...
    // x축 스케일 생성
    var x = d3.scaleTime()
      .domain(d3.extent(data as Data[], d => d.date) as unknown as [number, number])
      .range([ 0, width ]);
    ...
    svg.append("path")
      ...
      .attr("d", line as ValueFn<SVGPathElement, Data[] | null, string>)
  }
